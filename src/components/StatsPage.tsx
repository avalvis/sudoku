...
import { ArrowRight, BookOpen, Check, Clock3, Feather } from 'lucide-react';
...
function ResultJournal({ results }: { results: GameResult[] }) {
  ...
  if (!results.length) return <div className="journal-empty"><Feather size={24} strokeWidth={1.3} /><div><h3>Your next chapter is unwritten.</h3><p>Finished and abandoned attempts will appear here. A paused game stays in progress.</p></div></div>;
  return <>
...
export function StatsPage() {
  ...
  return <main id="main-content" tabIndex={-1} className="stats-page">
    ...
    <p className="stats-mode-note">{practice ? 'Practice results have their own record and never affect your normal-game statistics.' : 'Normal games only. Games continued in practice are counted in the Practice journal.'}</p>
