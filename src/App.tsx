...
import { Grid3X3, Moon, Sun, Volume2, VolumeX } from 'lucide-react';
...
function ClassicWorkspace({ open }: { open: (dialog: RequestedDialog) => void }) {
  ...
  return <main id="main-content" tabIndex={-1} className="main-content">
    <div className="page-heading"><div><span className="eyebrow">{daily ? `The daily edition / ${puzzleId.slice(6)}` : `The classic collection / No. ${edition}`}</span><h1>{daily ? 'A daily ritual' : 'Sudoku'}<span className="title-period">.</span></h1></div><p>A quiet moment.<br /><em>A sharper mind.</em></p></div>
...
export default function App() {
  ...
  useGameLifecycle(route === 'Classic' || route === 'Daily');
  ...
  return <MotionConfig reducedMotion="user"><a className="skip-link" href="#main-content" onClick={event => { event.preventDefault(); document.getElementById('main-content')?.focus(); }}>Skip to content</a>
    ...
    {!hydrated ? <main className="loading-page"><span className="eyebrow">Opening your edition…</span></main> : route === 'Classic' || route === 'Daily' ? <ClassicWorkspace open={setDialog} /> : route === 'Stats' ? <StatsPage /> : <ArchivePage />}
